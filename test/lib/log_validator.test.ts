...
import { parseLogIssues, buildValidationPrompt, selectRelevantFiles, LogValidator } from '../../src/lib/log_validator';
import type { LogIssue, CodeSnippet } from '../../src/lib/log_validator';
...
describe('parseLogIssues (pure)', () => {
	it('extracts ai_quality issues', () => {
		...
		const issues = parseLogIssues(line, 'test.log');
	...
	it('extracts dependency issues', () => {
		...
		const issues = parseLogIssues(line, 'workflow.log');
		expect(issues).toHaveLength(1);
	...
	it('extracts [WARNING] issues', () => {
		...
		const issues = parseLogIssues(line, 'w.log');
		expect(issues).toHaveLength(1);
	...
	it('returns empty array for clean log lines', () => {
		expect(parseLogIssues('✓ Step completed in 100ms', 'w.log')).toHaveLength(0);
		expect(parseLogIssues('[2026-01-01T00:00:00.000Z] Starting workflow...', 'w.log')).toHaveLength(0);
		expect(parseLogIssues('', 'w.log')).toHaveLength(0);
	});

	it('parses null timestamp when not present', () => {
		const issues = parseLogIssues('⚠ [WARNING] no timestamp here', 'w.log');
	...
	it('parses only one issue per line (first matching pattern wins)', () => {
		...
		expect(parseLogIssues(line, 'w.log')).toHaveLength(1);
		expect(parseLogIssues(line, 'w.log')[0].severity).toBe('error');
	});

	it('is deterministic — same input always produces same output', () => {
		const a = parseLogIssues(SAMPLE_LOG, 'step_04.log');
		const b = parseLogIssues(SAMPLE_LOG, 'step_04.log');
		expect(a).toEqual(b);
	});

	it('uses default source when not provided', () => {
		const issues = parseLogIssues('⚠ [WARNING] test');
...
describe('buildValidationPrompt (pure)', () => {
	it('includes all issue messages in the prompt', () => {
		const prompt = buildValidationPrompt(SAMPLE_ISSUES, []);
	...
	it('includes code snippet paths and content', () => {
		const prompt = buildValidationPrompt([], SAMPLE_SNIPPETS);
	...
	it('is deterministic', () => {
		const a = buildValidationPrompt(SAMPLE_ISSUES, SAMPLE_SNIPPETS);
		const b = buildValidationPrompt(SAMPLE_ISSUES, SAMPLE_SNIPPETS);
		expect(a).toBe(b);
	});
});
...
describe('selectRelevantFiles (pure)', () => {
	...
	it('scores files matching issue keywords higher', () => {
		...
		const result = selectRelevantFiles(paths, issues, 4);
	...
	it('respects maxFiles limit', () => {
		...
		expect(selectRelevantFiles(paths, issues, 2)).toHaveLength(2);
	});

	it('returns empty array when allPaths is empty', () => {
		expect(selectRelevantFiles([], SAMPLE_ISSUES, 10)).toHaveLength(0);
	});

	it('returns up to maxFiles even with no keyword matches', () => {
		const result = selectRelevantFiles(paths, [], 2);
	...
	it('is deterministic given same inputs', () => {
		...
		expect(selectRelevantFiles(paths, issues, 3)).toEqual(selectRelevantFiles(paths, issues, 3));
	});
});
...
describe('LogValidator', () => {
	beforeEach(() => {
		...
		mockReaddir.mockImplementation((dir: string, _opts: unknown) => {
	...
	it('sends a non-empty prompt to the model', async () => {
		const validator = new LogValidator();
		await validator.validate('/logs', '/src');
		const [promptArg] = mockSend.mock.calls[0] as [string];
	...
	it('respects maxCodeFiles option', async () => {
		...
		mockReaddir.mockImplementation((dir: string, _opts: unknown) => {
			...
			return Promise.resolve(
				['a.ts', 'b.ts', 'c.ts', 'd.ts', 'e.ts'].map((n) => ({
					name: n, isFile: () => true, isDirectory: () => false,
				})),
			);
		});

		const validator = new LogValidator({}, { maxCodeFiles: 2 });
		await validator.validate('/logs', '/src');

		const [prompt] = mockSend.mock.calls[0] as [string];
	...
	it('truncates files exceeding maxCharsPerFile', async () => {
		...
		mockReadFile.mockImplementation((filePath: string) => {
			if (String(filePath).endsWith('.log')) return Promise.resolve('⚠ [WARNING] test issue');
			return Promise.resolve(longContent);
		});

		const validator = new LogValidator({}, { maxCharsPerFile: 100 });
		await validator.validate('/logs', '/src');
