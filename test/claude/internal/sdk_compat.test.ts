 ...
 */
function loadFreshModule(sdkStub: Record<string, unknown>): SdkCompatModule {
	jest.doMock('@anthropic-ai/claude-agent-sdk', () => sdkStub);
	jest.resetModules();
	// eslint-disable-next-line @typescript-eslint/no-require-imports
	return require('../../../src/claude/internal/sdk_compat') as SdkCompatModule;
}
...
describe('getClaudeStartup', () => {
	...
	it('throws when startup is not exported by the SDK', async () => {
		...
		await expect(getClaudeStartup()).rejects.toThrow(
...
describe('getClaudeDeleteSession', () => {
	...
	it('throws when deleteSession is not exported by the SDK', async () => {
		...
		await expect(getClaudeDeleteSession()).rejects.toThrow(
