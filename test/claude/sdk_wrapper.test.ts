import { ClaudeSdkWrapper } from '../../src/claude/sdk_wrapper';
import { ClaudeSDKError } from '../../src/claude/errors';
...
jest.mock('@anthropic-ai/claude-agent-sdk', () => ({
	...
	startup: (...args: unknown[]) => mockStartup(...args),
...
async function* makeGen(messages: object[]) {
...
describe('ClaudeSdkWrapper', () => {
	...
	describe('isAvailable', () => {
		it('should return true when the SDK stub is loaded', () => {
			expect(ClaudeSdkWrapper.isAvailable()).toBe(true);
		});
	});
	...
	describe('warmup', () => {
		it('should call startup() and mark the wrapper as warmed', async () => {
			const wrapper = new ClaudeSdkWrapper({ model: 'claude-sonnet-4-5', cwd: '/tmp' });
		...
		it('should pass wrapper defaults to startup options', async () => {
			const wrapper = new ClaudeSdkWrapper({ model: 'claude-haiku-4-5', maxTurns: 3 });
		...
		it('should forward initializeTimeoutMs to startup', async () => {
			...
			expect(mockStartup).toHaveBeenCalledWith(
				expect.objectContaining({ initializeTimeoutMs: 5000 }),
			);
		});
	});
	...
	describe('run', () => {
		...
		it('should pass prompt and merged options to query()', async () => {
			const wrapper = new ClaudeSdkWrapper({ model: 'claude-sonnet-4-5', cwd: '/my/dir' });
		...
		it('should throw ClaudeSDKError when result subtype is an error', async () => {
			...
			const wrapper = new ClaudeSdkWrapper();
			await expect(wrapper.run('Fail please')).rejects.toThrow(ClaudeSDKError);
		});
