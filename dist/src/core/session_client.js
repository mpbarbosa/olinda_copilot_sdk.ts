...
class CopilotSdkWrapper {
    ...
    static isAvailable() {
        try {
            if (!copilot_sdk_1.CopilotClient || typeof copilot_sdk_1.CopilotClient !== 'function')
                return false;
            const probe = new copilot_sdk_1.CopilotClient();
            return probe !== null;
        }
        catch {
            return false;
     ...
     */
    async initialize() {
        this._client = new copilot_sdk_1.CopilotClient();
        ...
        try {
            ...
            if (this._authenticated) {
                try {
                    ...
                    logger_js_1.logger.debug(`Could not fetch available models: ${msg}`);
                }
                const sessionConfig = {
                    model: this._model,
    ...
    async send(prompt, timeoutMs) {
        if (!this._session) {
            throw new errors_js_1.SystemError('No active session. Call initialize() first.');
        }
    ...
    async recreateSession() {
        ...
        this._sendQueue = Promise.resolve();
        logger_js_1.logger.info('[SDK] Client and session recreated for retry');
    }
    /**
    ...
    // --------------------------------------------------------------------------
    // Session management & status — wraps CopilotClient methods (v0.5.1)
    // --------------------------------------------------------------------------
    /**
    ...
    async resumeSession(sessionId, config) {
        ...
        this._session = await client.resumeSession(sessionId, config ?? { onPermissionRequest: copilot_sdk_1.approveAll });
        this._sendQueue = Promise.resolve();
    }
    /**
    ...
    _requireClient() {
        if (!this._client) {
            throw new errors_js_1.SystemError('No active client. Call initialize() first.');
        }
        return this._client;
    }
    /** Performs a single serialised sendAndWait call. */
    async _doSend(prompt, timeoutMs) {
...
exports.CopilotSdkWrapper = CopilotSdkWrapper;
