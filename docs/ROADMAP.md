...
| `@github/copilot-sdk` semver review at 1.0 | Currently pinned as `^0.1.32` (resolves to `>=0.1.32 <0.2.1`). When the SDK reaches `1.0.0`, reassess the range: consider exact pinning or `~` to guard against unexpected breaking changes in a newly-stable API. Track the [copilot-sdk releases](https://github.com/github/copilot-sdk/releases) page. |
