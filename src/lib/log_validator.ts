...
import { CopilotSdkWrapper, type CopilotSdkWrapperOptions } from '../core/session_client.js';
...
// ---------------------------------------------------------------------------
// Issue detection patterns (ordered by specificity)
// ---------------------------------------------------------------------------
 ...
 * @pure
 */
export function parseLogIssues(logContent: string, source = 'unknown'): LogIssue[] {
 ...
 * @pure
 */
export function buildValidationPrompt(issues: LogIssue[], codeSnippets: CodeSnippet[]): string {
	const issueLines = issues
		.map(
			(i, idx) =>
				`${idx + 1}. [${i.severity.toUpperCase()}] ${i.message}` +
...
export function selectRelevantFiles(
	...
	for (const issue of issues) {
		...
		for (const m of matches) keywords.add(m[0].toLowerCase());
	}
...
export class LogValidator {
	private readonly _wrapperOptions: CopilotSdkWrapperOptions;
	private readonly _maxCodeFiles: number;
	private readonly _maxCharsPerFile: number;
	...
	constructor(
		...
		validatorOptions: LogValidatorOptions = {},
	) {
	 ...
	 */
	async validate(logDir: string, codebaseDir: string): Promise<string> {
	...
	private async _findLogFiles(dir: string): Promise<string[]> {
		const found: string[] = [];
	...
	private async _readAllIssues(logDir: string): Promise<LogIssue[]> {
	...
	private async _findSourceFiles(dir: string): Promise<string[]> {
		...
		const entries = await readdir(dir, { withFileTypes: true });
		for (const entry of entries) {
			if (this._excludeDirs.includes(entry.name)) continue;
			const full = join(dir, entry.name);
			if (entry.isDirectory()) {
	...
	private async _sampleCodebase(
		...
		for (const filePath of selected) {
			...
			const content =
				raw.length > this._maxCharsPerFile
