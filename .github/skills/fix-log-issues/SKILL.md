...
1. Run `cd "$project_root" && npm run lint:md` to confirm no markdown violations were introduced.
...
1. Run `cd "$project_root" && npm run lint:md` to identify any remaining violations.
1. For each remaining violation, edit the file manually to comply with the
   rule referenced in the `Description` field.
1. Re-run `cd "$project_root" && npm run lint:md` and confirm zero violations for the affected
   file(s).
...
1. Run `cd "$project_root" && npm ci && npm run build && npm test` to confirm nothing regressed.
...
1. Re-run `cd "$project_root" && npx tsc --noEmit` and confirm zero new errors.
...
1. Determine whether the code or the documentation is the source of truth.
   ...
   - If a directory was added accidentally: remove it or move it to the
     correct location.
1. Run `cd "$project_root" && npm run lint:md` after editing any documentation.
...
1. Run `cd "$project_root" && npm run lint:md` to confirm no violations were introduced.
...
the results to `$project_root/docs/ROADMAP.md` under a
...
Roadmap updated in $project_root/docs/ROADMAP.md.
```
...
- `$project_root/docs/ROADMAP.md` — output: roadmap updated with results
