...
1. **Workflow summary** (`$project_root/.ai_workflow/summaries/**/workflow_summary.md`) —
...
- **Undocumented directories** — run `ls <path>` and check whether the
  ...
  `docs/` file.
- **Markdown formatting violations** — inspect the file manually and check
  whether the violation still appears.
- **Dependency warnings** — run `cd "$project_root" && npm install --dry-run` and compare output.
- **JavaScript issues** — run `cd "$project_root" && npm run validate` and compare.
...
1. Write `$project_root/.ai_workflow/plan.md` using the format described above.
